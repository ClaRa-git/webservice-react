...
const Subscription = () => {
  ...
  //méthode qui recupère le choix de l'abonnement
  const handleSubscription = async (stripePriceId) => {
    try {
      //on récupere l'email de l'utilisateur en localstorage
      const email = userInfo.email;
      setIsLoading(true);

      const response = await axios.post(`${API_ROOT}/create-checkout-session`, {stripePriceId, email});
